...
| Module | Status | Notes |
| --- | --- | --- |
| `token-pilot-core` | Basic implementation complete | Domain records, pricing, calculator, registry, ledger manager, pricing snapshots, token count results, and a preflight cost-bound projection; versioned model snapshots and context admission are still required before reservation |
